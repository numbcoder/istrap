...
/**
 *    Istrap.js 0.1.1
 *    (c) 2011 gene@ihaveu.net
 *    Istrap.js may be freely distributed under the MIT license.
...
var Istrap = (function(window, $, undefined){
  ...
  //_bind, 可以将执行环境的this，用参数方式带入
	Function.prototype._bind = function(object){
		...
		var self = this, args = _slice.call(arguments, 1);
		return function(){
      var newArgs = args.concat(_slice.call(arguments, 0));
      newArgs.splice(1, 0, this);
			self.apply(object, newArgs);
		}
  };
  ...
  //默认dom容器
  var container = 'body';
  var delegateEvents = ['swipe', 'swipeLeft', 'swipeRight', 'swipeUp', 'swipeDown', 'doubleTap', 'tap', 'singleTap', 'longTap', 'click', 'dblclick', 'mousedown', 'mouseup', 'mousemove', 'mouseover', 'mouseout', 'mouseenter', 'mouseleave', 'submit'];
  ...
  */
  I.Controller = function(options){
    ...
    if(!this.constructor.isDetected){
      this._detectEvents(this.events, this.customEvents);
    }
    
		//this.fetch();
    if(this.pageSelector){
      this.page = $(this.pageSelector, this.container)[0] || $(this.pageSelector)[0];
      if(this.page){
        this.bindEvents(this.constructor.customEvents);
      }
    }
    
    this.bindEvents(this.constructor.events, true);
  ...
  _extend(I.Controller.prototype, I.Events, {
    ...
    append: function(container, child){
      ...
      this.bindEvents(this.constructor.customEvents, false, child[0]); 
    },

		//批量绑定事件
    bindEvents : function(events, delegate, dom) {
      if (!events) return;
      ...
      for (var key in events) {
        ...
        method = method._bind(this);
				if(delegate){
					$(dom).on(eventName, selector, method);
				}else{
					$(selector, dom).on(eventName, method);
				}
      }
    },

    //对事件进行预处理，把不能委托的事件，移到 customEvents  
    _detectEvents: function (events1, events2) {
      var events = _extend({}, events1);
      var customEvents = _extend({}, events2);
      
      var key, eventName;
      for (key in events) {
        eventName = key.match(eventSplitter)[1];
        if(eventName && delegateEvents.indexOf(eventName) == -1){
          customEvents[key] = events[key];
          delete events[key];
        }
      }

      this.constructor.events = events;
      this.constructor.customEvents = customEvents;
      this.constructor.isDetected = true;
    }
	...
	*/
	I.changePage = function(hash, options){
		...
		if (!hash && router.defaultHash) {
			hash = decodeURI(router.defaultHash);
		}
      ...
      return;
		...
		if( hashchange ){
			if (router.pushState) {
				window.history.pushState(state, '', router.prefix + encodeURI(hash));
			}else{
				changeHashSilent = true;
				window.location.hash = router.prefix + encodeURI(hash);
			}
		}
...
//添加对 CommonJS Module 支持
if (typeof exports !== 'undefined') {
  exports.Istrap = Istrap;
}else{
  window.Istrap = Istrap;
  'I' in window || (window.I = Istrap);
} 
